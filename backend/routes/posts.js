...
// Helper to transform a raw post row
const transformPost = (post, userId = null) => ({
  id: post.id,
  userId: post.user_id,
...
// Helper to get like/comment counts and liked status for posts
const enrichPosts = async (posts, userId) => {
  ...
  const { data: userLikes } = userId ? await supabaseAdmin
    .from('post_likes')
    .select('post_id')
    .in('post_id', postIds)
...
// GET /posts/feed - Global feed (all posts, newest first)
...
export const getComments = async (req, res) => {
  try {
    ...
    return res.status(200).json({ comments: transformed });
  } catch (error) {
